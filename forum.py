    ...
    def beta(self):
        ...
        for line in data.readlines():
            ...
            if line.find('<h5>') is not -1:
                ...
                dict[counter] = {'counter':counter, 'chapter':chapter, 'subject':subject, 
                'profile':profile, 'date':date, 'post':post}
            ...
            quoteheader = line.find('<div class="quoteheader">') #hata verebilir.
            if quoteheader is not -1: quoteheader = 1
            strong = line.find('</strong>')
            if strong is not -1: strong = 1
            if list_post is not -1:
                #print (counter, line, '\n', {'strong':strong, 'bbc_code':bbc_code, 'codeheader':codeheader, 'smfSelectText':smfSelectText, 'topslice_quote':topslice_quote, 'bbc_link':bbc_link, 'bbc_img':bbc_img, 'bbc_size':bbc_size, 'botslice_quote':botslice_quote, 'quoteheader':quoteheader})
                if bbc_code is -1 and codeheader is -1 and smfSelectText is -1 and topslice_quote is -1 and bbc_link is -1: 
                    if bbc_img == -1 and bbc_size == -1 and botslice_quote == -1 and quoteheader == -1 and strong is -1:
                        post = 'fnc:1','testediliyor', cut(line, '<div class="list_posts">' ,'<END>')
                        dict[int(counter)] = {'counter':counter, 'chapter':chapter, 'subject':subject, 'profile':profile, 'date':date, 'post':post}
                        continue
                if bbc_code is -1 and codeheader is -1 and smfSelectText is -1 and topslice_quote is -1 and bbc_link is 1: #target
                    if bbc_img == -1 and bbc_size == -1 and botslice_quote == -1 and quoteheader == -1 and strong is -1:
                        post = 'fnc:2','testediliyor', cutstring(line, 'class="bbc_link" target="_blank"' ,'</a>', '<END>')
                        dict[int(counter)] = {'counter':counter, 'chapter':chapter, 'subject':subject, 'profile':profile, 'date':date, 'post':post}
                        continue
                if bbc_code is -1 and codeheader is -1 and smfSelectText is -1 and topslice_quote is 1 and bbc_link is 1: #target
                    if bbc_img == -1 and bbc_size == -1 and botslice_quote == 1 and quoteheader == 1 and strong is -1:
                        post = 'fnc:3','testediliyor', 'data hatalı olabilir!', cutstring(line, '<div class="list_posts">' ,'class="bbc_link" target="_blank">', '</a>')
                        dict[int(counter)] = {'counter':counter, 'chapter':chapter, 'subject':subject, 'profile':profile, 'date':date, 'post':post}
                        continue
                if bbc_code is -1 and codeheader is -1 and smfSelectText is -1 and topslice_quote is 1 and bbc_link is -1: #target
                    if bbc_img == -1 and bbc_size == -1 and botslice_quote == 1 and quoteheader == 1 and strong is -1:
                        post = 'fnc:4', 'testediliyor', cut(line, '<div class="botslice_quote"><END><END>', '<END>' )
                        dict[int(counter)] = {'counter':counter, 'chapter':chapter, 'subject':subject, 'profile':profile, 'date':date, 'post':post}
                        continue
                if bbc_code is -1 and codeheader is -1 and smfSelectText is -1 and topslice_quote is -1 and bbc_link is 1:
                    if bbc_img == -1 and bbc_size == -1 and botslice_quote == -1 and quoteheader == -1 and strong is 1:
                        puf =  copy.copy(line)
                        puf = puf.replace('<strong>','').replace('</strong>','') #strong kendi icinde temizlendi
                        puf = cutreplace(puf, '<a href=', '</a>')
                        post = 'fnc:5','testediliyor', cut(puf, '<div class="list_posts">', '<END>')
                        dict[int(counter)] = {'counter':counter, 'chapter':chapter, 'subject':subject, 'profile':profile, 'date':date, 'post':post}
                        continue
        return dict
recent = recent()
